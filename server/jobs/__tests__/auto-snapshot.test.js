...
describe('createSettlementZip — adjustment 라운드 Subagent 3 (한국어 3파일)', () => {
  ...
  // 4. 메뉴 10행 모두 (ID 순). menu_update 라운드: 후라이드…양념 소스.
  it('★ 정산서.txt 메뉴 10행 ID 순 (menu_update — 후라이드~양념 소스)', async () => {
    const db = freshDb();
    const buf = await createSettlementZip(db, { operating_date: '2026-05-20' });
    const files = await unzipBuffer(buf);
    const txt = files['정산서-2026-05-20.txt'].toString('utf8');
    // ID 1~10 — menu_update 라운드 (신규 2종 생수/양념 소스 추가).
    for (const name of ['후라이드', '양념', '뿌링클', '감자튀김', '뿌링감자튀김', '칠리스', '콜라', '사이다', '생수', '양념 소스']) {
      expect(txt).toContain(name);
    }
    // ID 순 — '후라이드' 위치가 '양념 소스' 위치보다 앞.
    expect(txt.indexOf('후라이드')).toBeLessThan(txt.indexOf('양념 소스'));
  });

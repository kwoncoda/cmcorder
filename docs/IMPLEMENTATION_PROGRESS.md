...
## design_fix_v4 라운드 (`design_fix_v4`, 2026-05-19 → 5/20 운영 시작 직전)

사용자 명시 두 건 — ① 홈 카테고리 바 위 좌석 배치도 CTA(`TableMapCTA`) 신설 + ② 포장(takeout) 주문은 `READY → SETTLED` 직접 전이로 분기. 새 ADR 신설 없이 ADR-024/025 보존 범위. 3 서브에이전트 분담.

| 서브에이전트 | 담당 | 커밋 | 작업 문서 |
|---|---|---|---|
| SA-1 | 홈 메뉴 페이지 TableMapCTA organism + `<Link to="/map">` + MenuPage 통합 + components.css | `da50102` | `docs/tasks/2026-05-19-design-fix-v4-table-map-cta.md` |
| SA-2 | order-state.js `opts.deliveryType` (takeout READY→SETTLED 합법 / READY→DINING 방어선) + admin.js POST transition delivery_type 전달 + AdminCardColumn `getActionsForOrder` + order-repo takeout SETTLED 회귀 | `68fda3f` | `docs/tasks/2026-05-19-design-fix-v4-takeout-settled.md` |
| SA-3 | 전체 회귀 매트릭스 검증 (docker test/lint/build + curl 사이드체크) + P3 권고 1 (takeout READY→CANCELED fall-through 명시 회귀) + CLAUDE.md / DECISIONS.md / IMPLEMENTATION_PROGRESS.md / 작업 종합 로그 동기화 | (본 커밋) | `docs/tasks/2026-05-19-design-fix-v4-summary.md` |

**테스트 추이 (design_fix_v4):** 1358 (design_fix_v3 종료) → 1369 (SA-1: TableMapCTA 8 + MenuPage 통합 3) → **1385** (SA-2: order-state 9 + admin 4 + AdminCardColumn 2 + order-repo 1, +회귀 보강 1). SA-3은 기존 테스트에 expect 1 추가(P3 권고 — takeout READY→CANCELED fall-through 명시) → 카운트 유지 1385/1385. lint 0 errors (3 pre-existing warnings 유지). production 번들 305.82 kB (gzip 95.24 kB), build 6.4s. 운영 경로(`curl -sI`): `/`, `/menu`, `/api/business-state` 200 OK / `/map` 301 → `/map/` 200 (Express express.static 디렉토리 redirect, 변경 없음).

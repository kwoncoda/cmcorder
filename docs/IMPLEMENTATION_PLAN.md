...
- [x] **TDD 흐름:**
  ...
  2. GREEN: 기본 구현
  3. REFACTOR: forwardRef + ...rest 패턴 (ADR-028 변경, 2026-05-15 — react-hook-form 미채택. controlled inputs 호환은 그대로 유지)
- **완료 기준:**
...
- **완료 기준:**
  - 은행 6개 옵션 + "기타" 직접 입력
  - "다른 이름 이체" 체크 시 추가 이름 input 활성 — `useState` controlled (ADR-028 변경, 2026-05-15 — react-hook-form 미채택)
...
- **TDD 흐름:**
  ...
  2. GREEN: controlled useState + 학번 정규식 `^\d{2}\d{2}37\d{3}$` 또는 외부인 (ADR-028, 2026-05-15)
...
- **완료 기준:**
  - **useState controlled + zod** (ADR-028 변경, 2026-05-15 — react-hook-form 미채택). Task 4.4와 동일 패턴
  - **"다른 이름 이체" 체크 시 조건부 필드는 `useState`** (§3.5 7조)
...
- [x] 사용자 번들 ≤ 250KB gzip (React + Router + Zustand + zod + 코드 — react-hook-form 미채택 ADR-028, 실측 89.88 KB)

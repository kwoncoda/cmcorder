...
// - design_fix v5 — "다른 이름으로 이체" 체크박스 + otherName Input 제거 (사용자 요구).
//   서버 contract 보존을 위해 handleSubmit payload 에서 useOtherName=false, otherName=null 고정 전송.
...
const TransferReportForm = forwardRef(function TransferReportForm(
  ...
  const handleSubmit = (e) => {
    ...
    markAllTouched();
    if (!isValid) return;
    // design_fix v5 — "다른 이름으로 이체" UI 제거. 서버 contract 보존을 위해 useOtherName=false 고정.
    onSubmit?.({
      bank,

...
- **coupon-tab-scope-toggle (2026-05-21, D+1 운영 중)**: 어드민 쿠폰 탭에 「전체 누적 / 오늘」 토글 추가. 기본 모드 = 전체 누적 (5/20+5/21 양일 합산). `listCouponUsage({ operating_date: 'all' })` 분기로 WHERE 절 제거 — 기존 호출자(auto-snapshot ZIP `:476`)는 항상 구체 일자 전달이라 영향 0. 라우트 `GET /admin/api/coupons/usage?date=all` 허용(정산 ZIP의 `?date=all` 거부 정책과 의도적 차이 — 본 라우트는 *조회 view* 라 합산 OK). 응답 행에 `operating_date` 필드 추가, UI는 "전체" 모드에서만 영업일 컬럼(MM-DD) 노출. 정산 요약/정산서/정산 ZIP/메뉴별 매출은 모두 자체 SQL로 `operating_date = ?` 필터링이라 정산 영향 0. `CouponsPage.jsx` 117줄 (≤120 가이드 유지). 회귀: `server/routes/__tests__/admin.test.js` coupons/usage +2건 (`?date=all` 양일 합산 + 오늘만 vs 전체 합산) + 기존 `?date=YYYY-MM-DD` 회귀에 `operating_date` 필드 검증 1줄 추가 + `src/pages/admin/__tests__/CouponsPage.test.jsx` +4건 (기본 ?date=all / 영업일 컬럼 / 토글 클릭 재호출 / aria-selected 전환). 전체 회귀 카운트 1485 → 1503.

...
**현재 단계:** 구현 완료 (2026-05-14). React 18 + Vite + Express 4 + SQLite. 1385/1385 tests passing (table_lock 라운드 2026-05-19 + Codex P1/P2 보완 + design_fix_v3 — minimap legend 삭제 + ALREADY_USED 가운데 모달 + design_fix_v4 — TableMapCTA + 포장 READY→SETTLED). **D-1 리허설 5/19 → D-day 5/20 16:30 운영 시작**. 이후 작업은 *수정·패치·운영 가이드 보강* 단계.
...
- **design_fix_v4 라운드 (2026-05-20, 사용자 요청)**: ① 홈 메뉴 페이지 카테고리 바 위 `TableMapCTA` 신규 organism — `Link to="/map"`로 기존 미니맵 모달 재사용. 헤더 작은 미니맵 버튼 유지(`header-map-link`). MenuPage ≤120줄 유지. ② 포장 주문은 `READY → SETTLED` 직접 전이 (DINING 건너뜀). `server/domain/order-state.js`의 `canTransition`/`transition`에 `opts.deliveryType` 추가 — `LEGAL_TRANSITIONS` 객체 자체는 변경 X (DONE 메타 회귀 유지). admin.js POST transition이 `order.delivery_type` 전달. `AdminCardColumn.jsx`는 `ACTION_BY_STATUS` 유지 + `getActionsForOrder(order)` 함수 신설. 회귀: `TableMapCTA.test.jsx` 8건 + `MenuPage.test.jsx` design_fix_v4 통합 3건 + `order-state.test.js` takeout/dineIn 분기 9건 + DONE 메타 회귀 유지 + `admin.test.js` POST transition 4분기 + `AdminCardColumn.test.jsx` READY 버튼 분기 2건 + `order-repo.test.js` takeout SETTLED 1건. 1385/1385 tests.
